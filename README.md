...
> 一套 **LSB（最低有效位）图片隐写工具**，支持在 PNG 图片中隐藏/提取秘密信息，人眼完全无法察觉。
> V1 基础版：纯文本隐写 ｜ V2 进阶版：AES-256-GCM加密 + 任意文件嵌入 + 纠错编码
...
一张 **400×300** 的PNG图可隐藏约 **45KB** 的数据。

> ⚠️ 必须保存为 **PNG**（无损压缩），JPEG有损压缩会破坏LSB数据。
...
3. 图片尺寸决定最大容量：`最大字节数 = (宽 × 高 × 3) / 8`（V2带纠错时再除以3）
